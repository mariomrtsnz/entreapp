...
test('POST /comments 201 (user)', async () => {
  const { status, body } = await request(app())
    .post(`${apiRoot}`)
    .send({ access_token: userSession, rating: 'test', content: 'test', poi: 'test', photos: 'test' })
...
test('PUT /comments/:id 401 (user) - another user', async () => {
  const { status } = await request(app())
    .put(`${apiRoot}/${comment.id}`)
    .send({ access_token: anotherSession, rating: 'test', content: 'test', poi: 'test', photos: 'test' })
  expect(status).toBe(401)
})
